# 해당 기사에서 네이버가 총 몇 번 나오는지 정규표현식을 이용해서 파이썬에서 확인하는 코드 작성

import re

article = """전국 언론 노동자들이 시민사회단체와 함께 네이버의 지역 차별을 규탄하고 개선을 촉구하는 무기한 릴레이 시위에 돌입했다.
...
naver = []
for data in re.finditer("네이버", article):
    naver.append(data.group())
